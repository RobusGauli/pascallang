...
begin_token = lambda: Token(BEGIN)
end_token = lambda: Token(END)
dot_token = lambda: Token(DOT)
id_token = lambda val: Token(ID, val)
assign_token = lambda: Token(ASSIGN)
semi_token = lambda: Token(SEMI)
...
class Lexer:
    ...
    #new added peek method to peek into the input buffer without consuming
    ...
    def _var_or_reserved_keywords(self):
        _var = ''
        while self.current_character is not None and self.current_character.isalpha():
            _var += self.current_character
            self.advance()
        return RESERVED_KEYWORDS.get(_var, id_token(_var))
    ...
    def get_next_token(self):
        ...
        if self.current_character.isalpha():
            #that means it might be variable or reserved keyword
            #depending on the witation it must releast the next token
            return self._var_or_reserved_keywords()
